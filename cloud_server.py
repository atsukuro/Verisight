# cloud_server.py (v1.2.1)
# 最終完成版：Windows環境での文字コード不一致(UnicodeDecodeError)を解決済み。
...
def upload_data():
    ...
    if not os.path.isdir(stimuli_folder_path):
        ...
        return jsonify({"error": f"Stimuli folder '{stimuli_folder_name}' not found"}), 404
    
    print(f"   - Triggering analysis...")
    command = [sys.executable, ANALYZER_PATH, "--csv", temp_csv_path, "--stimuli", stimuli_folder_path]
    
    try:
        result = subprocess.run(command, capture_output=True, text=True, encoding='cp932', check=False)
